...
const mockSchedule = {
  assignments: [
    { name: 'History Paper', dueDate: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString() },
    { name: 'Math Midterm', dueDate: new Date(Date.now() + 8 * 24 * 60 * 60 * 1000).toISOString() },
    { name: 'Science Project Phase 1', dueDate: new Date(Date.now() + 12 * 24 * 60 * 60 * 1000).toISOString() },
  ],
  events: [
    { name: 'Career Fair', date: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString() },
  ],
};
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await predictStressPeriod({
        ...
        pastStressPatterns: JSON.stringify(
          mockMoodLogs
